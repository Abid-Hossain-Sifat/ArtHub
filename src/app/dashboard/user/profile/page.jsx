...
const UserProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const loadUserStats = async () => {
      try {
        const response = await fetch(`${backendUrl}/orders?userId=${encodeURIComponent(user.id)}`);
  ...
  return (
    <div className="w-full min-h-screen bg-[#F8F9FC] p-4 sm:p-6 lg:p-10 font-sans text-slate-800">
      ...
      <div className="mb-6 sm:mb-8 max-w-[1440px] mx-auto flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-[#0F172A]">Account Settings</h1>
          <p className="text-slate-500 text-xs sm:text-sm mt-1">Manage your personal profile, security options, and purchase history.</p>
        </div>
        <Link href="/dashboard" className="w-full sm:w-auto text-center inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all">
          Back to dashboard
        </Link>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10 items-stretch">
          ...
          <div className="lg:col-span-2 flex flex-col sm:flex-row gap-6 sm:gap-10 items-center sm:items-start">
            ...
            <div className="flex-1 space-y-4 text-center sm:text-left w-full h-full flex flex-col justify-start pt-2 items-center sm:items-start">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2.5 justify-center sm:justify-start">
                <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">{profile.name}</h2>
              ...
              <div className="space-y-1 w-full">
                <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Bio / Note</h4>
                <p className="text-slate-600 text-xs md:text-sm font-normal leading-relaxed max-w-lg mx-auto sm:mx-0">
                  Premium member of ArtHub community. Collecting and exploring beautiful masterworks.
                </p>
              </div>
          ...
          <div className="w-full bg-[#F8F9FC]/80 border border-slate-100 rounded-2xl p-5 sm:p-6 space-y-6 flex flex-col justify-between">
            <div className="space-y-4">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider text-center sm:text-left">Purchase Summary</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3.5">
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-100 flex items-center justify-between shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-xs text-slate-500 font-semibold">Bought Artworks</p>
                  </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-100 flex items-center justify-between shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-xs text-slate-500 font-semibold">Total Investment</p>
                  </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-100 flex items-start gap-3 shadow-sm">
              ...
              <p className="text-[11px] text-slate-500 leading-normal">
                Thank you for supporting global independent artists! All your premium digital certificates are saved securely in your profile dashboard.
              </p>
            </div>
          </div>
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-slate-500">Change your display name, email address or avatar picture.</p>
                </div>
              ...
              <div className="mt-6 space-y-5">
                ...
                <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end pt-2">
                  <button onClick={() => setEditOpen(false)} className="rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors w-full sm:w-auto">
                    Cancel
                  </button>
                  <button
                    onClick={saveProfile}
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-slate-500">Provide authentication parameters securely below.</p>
                </div>
              ...
              <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                <button onClick={() => setPasswordOpen(false)} className="rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors w-full sm:w-auto">
                  Cancel
                </button>
                <button
