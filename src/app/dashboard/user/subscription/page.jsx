  ...
  return (
    <div className="space-y-8 w-full max-w-7xl mx-auto p-4 md:p-6 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
        {[1, 2, 3].map((index) => (
          <div key={index} className="bg-white rounded-3xl p-6 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.01)] flex flex-col justify-between min-h-[420px]">
  ...
  const subscriptionTiers = [
    {
      ...
      period: "forever",
      limit: "3 paintings",
      features: ["Access to public gallery", "Basic profile customization", "Standard support"],
    ...
    },
    {
      ...
      period: "per month",
      limit: "9 paintings",
      features: ["Expanded portfolio view", "Advanced analytics", "Priority email support", "No intrusive ads"],
    ...
    },
    {
      ...
      features: ["All Pro features included", "Featured artist badge", "24/7 Dedicated support", "Early access to new tools"],
    ...
    <div className="space-y-8 w-full max-w-7xl mx-auto p-4 md:p-6 min-h-screen bg-slate-50/50">
      ...
      <div className="text-center max-w-xl mx-auto space-y-2">
        ...
        <p className="text-slate-500 text-sm md:text-base">
          Choose the right plan to expand your art gallery and unlock elite features.
        </p>
      </div>
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <div className={`p-2.5 w-fit rounded-xl ${
                  tier.id === 'free' ? 'bg-slate-100' : tier.id === 'pro' ? 'bg-[#f5efff]' : 'bg-emerald-50'
                }`}>
                  {tier.icon}
                </div>
              ...
              <div className="flex items-baseline text-slate-800">
                <span className="text-3xl md:text-4xl font-extrabold tracking-tight">{tier.price}</span>
                <span className="ml-1 text-sm font-medium text-slate-400">/{tier.period}</span>
              </div>
              ...
              <ul className="space-y-3 pt-2">
                {tier.features.map((feature, idx) => (
                  <li key={idx} className="flex items-start gap-2.5 text-sm text-slate-500">
                    <Check className={`w-4 h-4 mt-0.5 shrink-0 ${tier.isPopular ? 'text-[#6211cf]' : 'text-slate-400'}`} />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
